 ...
 * Purpose: Research possibility of creating a rendering engine. 
 * Work with Microsoft game creation library(es)
 * Test of the particle system and effect creation
 */
...
/// creates a child class of the framework game class
/// </summary>
...
{      
    ...
    RenderTarget2D world_ui_layer;                                   // UI rendering here
    RenderTarget2D world_ui_mask_layer;                              // UI masking renering here
    ...
    public static SpriteFont small_font;                             // text font 1
    public static SpriteFont large_font;                             // text font 2
    ...
    public int screen_max_height 
        = GraphicsAdapter.DefaultAdapter.CurrentDisplayMode.Height;  // maximum height clip
    public int screen_max_width  
        = GraphicsAdapter.DefaultAdapter.CurrentDisplayMode.Width;   // maximum width clip
    ...
    protected override void Initialize()
    {
        ...
        engine = Engine.get_instance(ref spriteBatch,ref viewport, this);// create a singleton Engine object
    ...
    protected override void LoadContent()
    {
        ...
        // load icons into editor menus / load worlds
        engine.load_user_interface();
        engine.LoadContent(Content);
        // generate clouds + other world content
        foreach (WorldStruct w in engine.get_world_list().worlds)
        {
            w.world.LoadContent(this.Content, engine);
        }

        // deserialize (LOAD) world props - if files exist
        engine.deserialize_tree_data();
        engine.deserialize_grass_data();
        engine.deserialize_light_data();
        engine.deserialize_watergen_data();
    }
    ...
    /// </summary>
    /// <param name="gameTime">gametime - for frame time calculation</param>
...
// update section for engine textarea
        engine.engine_text_engine.clear_messages(); // always show only one message containing statistics
...
// prepare statistics
        statistics_text += "{rendered objects:}[orange] total: " + engine.number_to_KMB(engine.get_draw_calls()) + "[skyblue] current: " + (draw_call_current - draw_call_previous) + "[0,255,40] /nl ";
        statistics_text += "fps:[orange] " + string.Format("{0:0.0}", engine.get_fps()) + "[skyblue] frame:[orange] " + engine.get_frame_count() + "[skyblue] long[orange] frames[orange]: "
            + overtime_frames + "[red] " + " %[orange]: " + string.Format("{0:0.00}", ((float)overtime_frames / (float)engine.get_frame_count()) * 100) + "[skyblue] % /nl ";
        ...
        statistics_text += "{number of lights:}[orange] " + engine.get_world_list().get_current().get_number_of_lights() + " selected:[orange] " + engine.get_editor().get_selection_lights().Count + " /nl " 
            + " {wind speed:}[188,244,66] " + string.Format("{0:00.00}", engine.get_wind_speed()) + " {queued:}[188,244,66] " + string.Format("{0:00.00}", engine.get_wind_change()) + "" + engine.get_wind_time_until_next_change() + " /nl ";

        statistics_text += "rain chance: "+engine.get_rain_chance()+" number of clouds: " + engine.get_number_of_clouds_and_rain_clouds().X + " rain clouds: " + engine.get_number_of_clouds_and_rain_clouds().Y + " /nl ";
        statistics_text += "{number of particles:}[red] " + engine.get_particle_engine().count_particles() + " /nl ";
        ...
        if (engine.get_world_list().get_current().in_edit_mode())
            ...
            "[skyblue] element:[green] " + engine.get_editor().GUI.get_hovered_element_id() + "[skyblue] /nl action:[green] " + engine.get_editor().GUI.get_hovered_element_action_text() + "[skyblue]";
...
// end of the new statistics
    ...
    {
        // turn these back on if game crashes - clip the screen to the sizes that are power of two 
        GraphicsDevice.SamplerStates[0] = SamplerState.LinearClamp;
        GraphicsDevice.SamplerStates[1] = SamplerState.LinearClamp;
        graphics.ApplyChanges();
...
// UI render target - contains static positioned UI elements
        ...
        spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend);
            ...
            //engine.get_world_list().get_current().draw_world_prop_info_layer(engine, spritebatch);   // world post-processing stats  
...
// This section is independent from Shader effect - masking - game world needs to be fully visible unlike some of the UI
        ...
        spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend);
            if(engine.get_current_world().in_edit_mode())
                engine.get_editor().draw_post_processing(engine, spritebatch); 
...
// draw world map tiles
        ...
        spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend); // Set deferred mode to draw everything in minimal gpu operations. also switch textures in draw function as few times as possible
            engine.get_world_list().get_current().draw_map_tiles(engine, spritebatch);
        spriteBatch.End();
/// world props buffer
        GraphicsDevice.SetRenderTarget(world_tile_buffer);
        spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend);
            engine.get_world_list().get_current().draw_trees_part1(engine, spritebatch);
            engine.draw_clouds();
            engine.get_particle_engine().Draw(g, engine); // draw particles on top of the world layer
        spriteBatch.End();
...
// UI mask       
        spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend);
            // bug fix: eliminated internal transparency and made interface transparency universal in order to make sure multi-component elements get evenly transparent
    ...
    /// <summary>
    /// Draw the engine overlay, mouse pointer etc.
    /// </summary>
    ...
    public void draw_engine(GameTime g)
    {
        ...
        Vector2 length_fps = small_font.MeasureString("FPS: " + engine.get_fps().ToString());
        ...
        engine.xna_draw_outlined_text(
        "FPS: " + engine.get_fps().ToString(),
        new Vector2(viewport.Width - (length_fps.X + 5), 25), Vector2.Zero, Color.LawnGreen, Color.Black, small_font);
        spriteBatch.End();
    }
//===================================================================================================================
...
//===================================================================================================================
    /// <summary>
    /// Saves the world tile order ionto an xml file
    /// </summary>
            ...
            for (int i = 0; i < w.world.width; i++)
            {
                for (int j = 0; j < w.world.height; j++)
                {
                    if (!w.world.world_map[i, j].is_air())
                    {
                        ...
                        obj.block_name = (int)w.world.world_map[i, j].tile_id;
                        obj.block_x = i + 1;
                        obj.block_y = j + 1;
                        obj.height = 1;
                        obj.width = 1;
                        obj.water_content = w.world.world_map[i, j].water_units;
    ...
    /// <summary>
    /// create a circle texture (brightness sphere of point lights)
    /// </summary>
    /// <param name="radius">circle radius</param>
    /// <param name="color">light color</param>
    /// <param name="intensity">light brightness</param>
    /// <returns>texture2d graphic element</returns>
    ...
    /// <summary>
    /// create a circle texture (brightness sphere of point lights) but with no smoothing that would exist for lights and only shows the outline
    /// </summary>
    /// <param name="radius">circle radius</param>
    /// <param name="color">light color</param>
    /// <param name="intensity">light brightness</param>
    /// <param name="outline_only">removes the inner parts of the circle</param>
    /// <returns>texture2d graphic element</returns>
    ...
    /// <summary>
    /// Create a half circle for composite percentage pie chart
    /// </summary>
    /// <param name="radius">circle radius</param>
    /// <param name="color">circle color</param>
    /// <param name="intensity">color transparency</param>
    /// <returns>half circle texture</returns>
    ...
    public void Controls()
    {
        ...
        //end text input---------------------------------------------------------
            ...
            {
                // turn edit mode on in every world, Editor object is a single instance in Engine.
                foreach (WorldStruct w in engine.get_world_list().worlds)
                {
                    w.world.toggle_edit_mode();
                }
            ...
            {
                engine.get_world_list().change_current(engine);
            }
            ...
            if (engine.get_current_keyboard_state().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.LeftAlt) || engine.get_current_keyboard_state().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.RightAlt))
            {
                ...
                else if (engine.get_current_keyboard_state().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.W)
                && !engine.get_previous_keyboard_state().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.W))
                {
                    if (engine.get_world_list().get_current().in_edit_mode())
                        engine.get_editor().editor_command(engine, command.destroy_water_gen);
                }
                else if (engine.get_current_keyboard_state().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.L)
                && !engine.get_previous_keyboard_state().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.L))
                {
                    if (engine.get_world_list().get_current().in_edit_mode())
                        engine.get_editor().editor_command(engine, command.destroy_lights);
                }
                else if (engine.get_current_keyboard_state().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.T)
                && !engine.get_previous_keyboard_state().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.T))
                {
                    if (engine.get_world_list().get_current().in_edit_mode())
                        engine.get_editor().editor_command(engine, command.destroy_trees);
                }
                else if (engine.get_current_keyboard_state().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.G)
                && !engine.get_previous_keyboard_state().IsKeyDown(Microsoft.Xna.Framework.Input.Keys.G))
                {
                    if (engine.get_world_list().get_current().in_edit_mode())
                        engine.get_editor().editor_command(engine, command.destroy_grass);
                }
            }
    ...
    /// <summary>
    /// Udpate program window resolution
    /// </summary>
    ...
    /// <summary>
    /// Changes the program mode to fullscreen
    /// </summary>
